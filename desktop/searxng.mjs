...
async function download(url, dest, onLog, label) {
  ...
  const res = await fetch(url, { headers: { 'user-agent': 'Curiocity-Desktop' } });
...
async function resolvePythonURL() {
  const res = await fetch(PBS_RELEASE, {
    headers: { 'user-agent': 'Curiocity-Desktop' },
  });
...
# generated by Curiocity's desktop provisioner for out-of-git builds
