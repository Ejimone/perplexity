...
const bodySchema = z.object({
  /* Optional. The desktop app sends the model the user picked; a hosted deploy
     has no picker and no stored preference, so the server falls back to
     whatever provider it is configured with. Resolving it here also means the
     client never needs to enumerate providers just to ask a question. */
  chatModel: z
    .object({
...
export const POST = async (req: Request) => {
  try {
    ...
    if (!chatModel) {
      const available = await registry.getActiveProviders();
      const provider = available.find((p) => p.chatModels.length > 0);
      ...
      chatModel = { providerId: provider.id, key: provider.chatModels[0].key };
    }
